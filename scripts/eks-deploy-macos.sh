...
# Skip EKS cluster creation since it's already done
...
# Skip ECR repository creation since it's already done
...
echo "====== Final Deployment to EKS ======"

# Update the Kubernetes manifests with the ECR repository URI
# For macOS compatibility, create a temporary file with replacements
cat kubernetes/wordpress.yaml | sed "s|\${ECR_REPOSITORY_URI}|$ECR_REPOSITORY_URI|g" > kubernetes/wordpress-updated.yaml

# Apply the updated Kubernetes manifests
kubectl apply -f kubernetes/wordpress-updated.yaml
